...
#ifdef _USE_COLOR_EMPHASIZE_
    #define COLOR_EMPHASIZE (1)
#else
    #define COLOR_EMPHASIZE (0)
#endif
...
parseArg(int num, char *string[])
{
    ...
    targetStringSz_G   = strlen(targetString_G);
}
...
grepFile(void *arg) {
    ...
    long  line;
    #if COLOR_EMPHASIZE
    char  *p = NULL, *subStr[3] = {NULL, NULL, NULL};
    #endif
    ...
    while (leftSize > 0 && fgets(buf, LINEBUF, fp_status)) {
        #if COLOR_EMPHASIZE
        if ((p = strstr(buf, targetString_G)) != NULL)
        #else
        ...
        #endif
        {
            if ( file->outputPath == 0 ) {
                ...
                *(subStr[0] + (p - buf)) = '\0';
                subStr[1] = p;
                *(subStr[1] + targetStringSz_G) = '\0';
                *subStr[2]  =  p + targetStringSz_G;
                printf("%s(%ld):%s""\033[31m%s\033[32m""%s", 
                    file->fname, line, subStr[0], subStr[1], subStr[2]);
                #else
                printf("%s(%ld):%s", file->fname, line, buf);
                #endif
            }
        } 
        ret = strlen(buf);
